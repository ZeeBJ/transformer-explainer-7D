<script lang="ts">
	...
	// Accept params prop to suppress SvelteKit warning (we use $page.params instead)
	export let params: Record<string, string> = {};
	
	// Reference params to avoid unused prop warning
	$: _ = params;

	ort.env.wasm.wasmPaths = 'https://cdn.jsdelivr.net/npm/onnxruntime-web@1.23.0/dist/';
	ort.env.logLevel = 'error';

	// Configure transformers to use remote URL instead of local server
	// This prevents 404 errors when trying to load tokenizer files from local server
	// The library will automatically fallback to Hugging Face CDN, but we can disable local model lookup
	if (typeof window !== 'undefined') {
		// Disable local model lookup to prevent 404 errors
