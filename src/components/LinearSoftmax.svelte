...
>
	...
	>
		<div class="title-text flex w-max items-center gap-1">
			{$languageStore('linearSoftmax.title')}
				...
				{#each data as item, idx}
					...
					>
						...
						<Tooltip class="softmax-tooltip" type="light">
							{$languageStore('linearSoftmax.tokenId')}: <span class="number">{tokenIds[idx]}</span>
						</Tooltip>
					</div>
				{/each}
			</div>
			...
			{#if isSoftmaxExpanded}
				<div class="softmax-subtitle softmax-detail flex text-center text-xs opacity-0">
					<div class="title-box token-string !justify-end">
						<div class="title-text">{$languageStore('common.tokens')}</div>
					</div>
					<div class="title-box logits">
						...
						>
							{$languageStore('common.logits')} <EyeOutline class="icon text-gray-400" size="sm" />
						</div>
					</div>
					<div class="title-box scaled">
						<TextbookTooltip id="temperature"
							><div class="title-text">{$languageStore('common.scaledLogits')}</div></TextbookTooltip
						>
					</div>
					<div class="title-box sampling">
						<TextbookTooltip id="sampling"
							><div class="title-text">
								{$sampling.type === 'top-k' ? $languageStore('sampling.topK') : $languageStore('sampling.softmaxAndTopP')}
							</div></TextbookTooltip
						>
					</div>
					<div class="title-box probability">
						<div class="title-text mr-1">
							{$sampling.type === 'top-k' ? $languageStore('common.softmax') : $languageStore('common.normalization')}
						</div>
					</div>
				</div>
			{/if}
