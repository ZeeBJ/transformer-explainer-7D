<script lang="ts">
	...
	// Get translated textPages based on current language
	$: translatedTextPages = textPages.map((page) => {
		const t = $languageStore;
		const translationKeyBase = idToTranslationKey(page.id);
		const translationKey = `textbook.${translationKeyBase}`;
		const translatedTitle = t(`${translationKey}.title`) || page.title;
		const translatedContent = t(`${translationKey}.content`) || page.content;
		
		return {
			...page,
			title: translatedTitle !== `${translationKey}.title` ? translatedTitle : page.title,
			content: translatedContent !== `${translationKey}.content` ? translatedContent : page.content
		};
	});
</script>
...
<div class="floating-container" transition:fade={{ duration: 150 }}>
	<div
		...
		role="region"
		aria-label="Textbook card"
	>
		<div class="card-header">
			<div class="flex w-full items-center justify-between">
				<h3 class="text-lg font-semibold text-gray-900">
					{translatedTextPages[$textbookCurrentPage].title}
				</h3>
		...
		<div class="card-body">
			<div class="text-carousel">
				<div class="carousel-content">
					{#each translatedTextPages as page, index}
				...
				<TextbookNavigation textPages={translatedTextPages} {isMouseInCard} {isLeftSide} />
			</div>
		</div>
	</div>
</div>
