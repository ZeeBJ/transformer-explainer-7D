...
<div class="input-area" data-click="input-area">
	<form class="input-form" data-click="input-form">
		<ButtonGroup class="input-btn-group" size="sm">
			...
			>
				{$languageStore('common.examples')}<ChevronDownOutline class="pointer-events-none h-4 w-4 text-gray-500" />
			</button>
			...
			>
				<div class={`editable ${!$isModelRunning ? 'w-full' : ''}`}>
					<div
						...
						class="text-box"
						placeholder={$languageStore('inputForm.placeholder')}
				...
				{#if $isMobile}
					<span class="helper-text"
						>{$languageStore('inputForm.mobileHint')}</span
					>
				{:else if $isLoaded && $isFetchingModel}
					<span class="helper-text"
						>{$languageStore('inputForm.downloadingHint')}</span
					>
				{:else if exceedLimit}
					<span class="helper-text">{$languageStore('inputForm.wordLimit', { limit: wordLimit })}</span>
				{/if}
			</div>
		</ButtonGroup>
		<button
		...
		>
			{$languageStore('inputForm.generate')}
		</button>
	</form>
